...
-- FICHIER UNIQUE D'INSTALLATION ET DE MIGRATION
-- Safe à rejouer sur une installation existante (idempotent).
...
CREATE TABLE IF NOT EXISTS `op_verifications` (
  ...
  `display_of_num` VARCHAR(64) DEFAULT '',
...
CREATE TABLE IF NOT EXISTS `cordon_verifications` (
  ...
  `statut`      ENUM('ok','nok','retouche') DEFAULT 'ok',
...
INNER JOIN (
  ...
  -- OP 250_3
  SELECT 'OP 250_3',  94,  0 UNION ALL
  SELECT 'OP 250_3',  95,  1 UNION ALL
  SELECT 'OP 250_3',  96,  2 UNION ALL
  SELECT 'OP 250_3',  97,  3 UNION ALL
  -- OP 260_6
  SELECT 'OP 260_6', 338,  0 UNION ALL
  SELECT 'OP 260_6', 339,  1 UNION ALL
  -- OP 260_7
  SELECT 'OP 260_7', 340,  0 UNION ALL
  SELECT 'OP 260_7', 341,  1 UNION ALL
  SELECT 'OP 260_7', 342,  2 UNION ALL
  SELECT 'OP 260_7', 343,  3
...
-- ─────────────────────────────────────────────────────────────────────────────
-- Migrations (safe à rejouer — installations existantes et nouvelles)
-- ─────────────────────────────────────────────────────────────────────────────

-- 1. Ajoute 'retouche' au ENUM statut (sans effet si déjà présent)
ALTER TABLE `cordon_verifications`
  MODIFY COLUMN `statut` ENUM('ok','nok','retouche') DEFAULT 'ok';

-- 2. Ajoute display_of_num si absent
ALTER TABLE `op_verifications`
  ADD COLUMN IF NOT EXISTS `display_of_num` VARCHAR(64) NOT NULL DEFAULT '' AFTER `piece_num`;

-- 3. Supprime la contrainte UNIQUE uk_op_image si elle existe encore
SET @idx_exists = (
    SELECT COUNT(*) FROM information_schema.STATISTICS
    WHERE TABLE_SCHEMA = DATABASE()
      AND TABLE_NAME   = 'op_schema_images'
      AND INDEX_NAME   = 'uk_op_image'
);
SET @sql = IF(@idx_exists > 0,
    'ALTER TABLE op_schema_images DROP INDEX uk_op_image',
    'SELECT 1'
);
