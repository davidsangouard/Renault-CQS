...
function App() {
  ...
  React.useEffect(() => {
    (async () => {
      try {
        ...
        const ofsKeys = Object.keys(ofsData);
        if (ofsKeys.length === 0) {
          // First run: seed demo data
          const demo = makeDemoData(cfg);
          setOfs(demo);
          for (const ofData of Object.values(demo)) {
            await API.createOf(ofData).catch(() => {});
          }
        } else {
          setOfs(ofsData);
        }
  ...
  async function handleComplete(opKey, formData) {
    ...
    const ofId   = curOF || (formData.ofNum?.trim()) || ('SANS-OF-' + Date.now());

    if (curOF && ofs[curOF]) {
      // Update existing OF locally
      setOfs(prev => ({
        ...prev,
        [curOF]: {
          ...prev[curOF],
      ...
      if (base) {
        ...
        await API.saveVerif({
          ofId, opKey,
          organeId:    base.organeId,
          organeLabel: base.organeLabel,
          projetId:    base.projetId,
          projetLabel: base.projetLabel,
          date:        base.date,
          opsList:     base.opsList,
          opData:      { ...formData, status },
        }).catch(console.error);
      }
    }

    setCurOP(null); setCurPiece(null); setDraftOp(null); setView('home');
  }
  ...
  function handleNewVerif(opKey, piece) {
    ...
    setDraftOp(draft);
    setCurOF(null); setCurOP(opKey); setCurPiece(piece || null);
    setView('wizard');
  }
    ...
    <div style={{ minHeight:'100vh', background:'#f4f3ef', fontFamily:"'DM Sans', system-ui, sans-serif" }}>
      ...
      {userIpn && (view === 'home' || view === 'select') && (
        <ProjetSelectScreen config={config} ofs={ofs} userIpn={userIpn} initial={filter}
          ...
          onSelectOF={(id, opKey, piece) => { setCurOF(id); setCurOP(opKey||null); setCurPiece(piece||null); setView('wizard'); }}
