...
function makeDefaultSousEns(opDef) {
  const pre = (opDef?.sousEns || []).slice(0, SE_ROWS).map(se => ({
    opNum: se.label || '', numD: '', numG: '', retoucheD: false, retoucheG: false,
  }));
  while (pre.length < SE_ROWS) pre.push({ opNum: '', numD: '', numG: '', retoucheD: false, retoucheG: false });
  return pre;
}
...
function makeEmptyOp(opDef) {
  ...
  return {
    ...
    sousEnsembles: makeDefaultSousEns(opDef),
    cordons, conformite: null, commentaire: '',
  };
}

function makeEmptyOF(id, organeId, cfg) {
  const config = cfg; if (!config) return null;
...
function makeDemoData(cfg) {
  const config = cfg; if (!config) return {};
...
Object.assign(window, {
  DEFAULT_CONFIG, findOrgane,
  SE_ROWS, makeDefaultSousEns,
