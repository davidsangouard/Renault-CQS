...
function fetchAllOfs(PDO $db): array {
    ...
    $verifs = $db->query("
        SELECT id, of_number, operation_id, status, piece_num, display_of_num, ipn, conformite, commentaire
    ...
    // All sous_ensemble_verifications indexed by op_verif_id (new structure: row_order + op_num)
    $sevRows = $db->query("
        SELECT op_verif_id, row_order, op_num, num_d, num_g, retouche_d, retouche_g
        FROM sous_ensemble_verifications
        ORDER BY op_verif_id, row_order
    ")->fetchAll();
    $sevsByVerif = [];
        ...
        foreach ($orgOps as $opDef) {
            ...
            if ($verif) {
                ...
                foreach ($cvsByVerif[$ovId] ?? [] as $cv) {
                    ...
                    // backward compat: old entries had statut='nok' + retouche=1 for retouche state
                    $cordons[$num] = ($cv['statut'] === 'nok' && $cv['retouche']) ? 'retouche' : $cv['statut'];
                }
                $seList = [];
                ...
                // Pad to 9 rows
                while (count($seList) < 9) {
                    $seList[] = ['opNum'=>'','numD'=>'','numG'=>'','retoucheD'=>false,'retoucheG'=>false];
                }
                $opsData[$opKey] = [
                    'status'        => $verif['status']           ?? 'todo',
                    'pieceNum'      => $verif['piece_num']         ?? '',
                    'ofNum'         => $verif['display_of_num']    ?? '',
                    'ipn'           => $verif['ipn']               ?? '',
                ...
                $opsData[$opKey] = [
                    ...
                    'commentaire'   => '',
                    'cordons'       => (object)[],
                    'sousEnsembles' => array_fill(0, 9, ['opNum'=>'','numD'=>'','numG'=>'','retoucheD'=>false,'retoucheG'=>false]),
                ];
            }
        }
...
function upsertSousEns(PDO $db, int $ovId, int $opId, array $seList): void {
    $db->prepare("DELETE FROM sous_ensemble_verifications WHERE op_verif_id=?")->execute([$ovId]);
    if (empty($seList)) return;
    $ins = $db->prepare(
        "INSERT INTO sous_ensemble_verifications
           (op_verif_id, row_order, op_num, num_d, num_g, retouche_d, retouche_g)
         VALUES (?, ?, ?, ?, ?, ?, ?)"
    );
    foreach ($seList as $i => $se) {
        $ins->execute([
            $ovId, $i,
            $se['opNum']    ?? '',
            $se['numD']     ?? '',
...
if ($method === 'POST') {
    ...
    foreach ($b['ops'] ?? [] as $opKey => $opData) {
        ...
        $db->prepare(
            "INSERT INTO op_verifications (of_number, operation_id, status, piece_num, display_of_num, ipn, conformite, commentaire)
             VALUES (?,?,?,?,?,?,?,?)
             ON DUPLICATE KEY UPDATE
               status=VALUES(status), piece_num=VALUES(piece_num), display_of_num=VALUES(display_of_num),
               ipn=VALUES(ipn), conformite=VALUES(conformite), commentaire=VALUES(commentaire), updated_at=NOW()"
        )->execute([
            $ofNumber, $opId,
            $opData['status']      ?? 'todo',
            $opData['pieceNum']    ?? '',
        ...
        if (!empty($cordons)) {
            ...
            foreach ($cordons as $num => $statut) {
                ...
                $retouche = ($statut === 'retouche') ? 1 : 0;
                $insCv->execute([$ovId, $statut, $retouche, $opId, (int)$num]);
            }
        }

        upsertSousEns($db, $ovId, (int)$opId, $opData['sousEnsembles'] ?? []);
    }
...
// ── DELETE: supprime un OF et toutes ses vérifications ───────────────────
if ($method === 'DELETE') {
    $db       = getDB();
    $ofNumber = trim($_GET['id'] ?? '');
    if (!$ofNumber) jsonErr('id requis');
    $db->prepare("DELETE FROM ordres_fabrication WHERE of_number=?")->execute([$ofNumber]);
    jsonOk(['ok' => true]);
}
