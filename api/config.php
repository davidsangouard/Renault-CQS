...
function syncProjets(PDO $db, array $projets): void {
    ...
    // Delete projects no longer in the list (only if no associated OFs)
    if (!empty($projCodes)) {
        $ph = implode(',', array_fill(0, count($projCodes), '?'));
        $toDelete = $db->prepare("
            SELECT p.id FROM projets p
            WHERE p.code NOT IN ($ph)
            AND NOT EXISTS (
                SELECT 1 FROM organes o
                JOIN ordres_fabrication f ON f.organe_id = o.id
                WHERE o.projet_id = p.id
            )
        ");
        $toDelete->execute($projCodes);
        foreach ($toDelete->fetchAll(PDO::FETCH_COLUMN) as $id) {
